...
interrupt void SCI_rx_interrupt(void)
{
    ...
    {
        // read byte
        data = SCI_MODUL.SCIRXBUF.bit.RXDT;
...
interrupt void SCI_tx_interrupt(void)
{
    ...
    else
    {
        ...
        {
            // first send MSB
            SCI_MODUL.SCITXBUF = 0x00FF & (*SCI_tx_ptr >> 8);
            SCI_tx_sent = SCI_tx_sent + 1;
            ...
            // then send LSB
            SCI_MODUL.SCITXBUF = 0x00FF & *SCI_tx_ptr;
            SCI_tx_sent = SCI_tx_sent + 1;
...
void SCI_init(long baud_rate)
{
    ...
    {
        sci_freq = CPU_FREQ / (2 * SysCtrlRegs.LOSPCP.bit.LSPCLK);
    }
    ...
    SCI_MODUL.SCIHBAUD = baud_H;
    SCI_MODUL.SCILBAUD = baud_L;

    // configure SCI peripherals to use FIFO
    SCI_MODUL.SCICTL1.bit.SWRESET = 0;

    SCI_MODUL.SCICCR.bit.STOPBITS = 0;
    ...
    #if SCI_DEBUG_MODE == 0
    SCI_MODUL.SCIPRI.bit.FREE = 0;
    SCI_MODUL.SCIPRI.bit.SOFT = 0;
    #else
    SCI_MODUL.SCIPRI.bit.FREE = 1;
    SCI_MODUL.SCIPRI.bit.SOFT = 1;
    #endif
    ...
    SCI_MODUL.SCIFFTX.bit.SCIFFENA = 1;
    SCI_MODUL.SCIFFTX.bit.TXFIFOXRESET = 1;
    ...
    EALLOW;
    PieVectTable.SCIRXINTA = &SCI_rx_interrupt;
    PieVectTable.SCITXINTA = &SCI_tx_interrupt;
    EDIS;
...
{
    //dam v buffer
    if ((SCI_Q_last_byte & 0x0001) == 0)
    {
        *(SCI_Q_buffer + (SCI_Q_last_byte>>1)) =  (*(SCI_Q_buffer + (SCI_Q_last_byte>>1)) & 0x00FF) | (element << 8);
    }
    else
    {
        *(SCI_Q_buffer + (SCI_Q_last_byte>>1)) =  (*(SCI_Q_buffer + (SCI_Q_last_byte>>1)) & 0xFF00) | (element);
    }
...
int SCI_queue_get(void)
{
    ...
    if (SCI_Q_nr_bytes > 0)
    {
        if ((SCI_Q_first_byte & 0x0001) == 0)
        {
            element = (*(SCI_Q_buffer + (SCI_Q_first_byte >> 1)) >> 8) & 0x00FF;
        }
        else
        {
            element = (*(SCI_Q_buffer + (SCI_Q_first_byte >> 1))) & 0x00FF;
        }
        // povecam kazalec
        SCI_Q_first_byte = SCI_Q_first_byte + 1;
        SCI_Q_nr_bytes = SCI_Q_nr_bytes - 1;
...
int SCI_get_packet(int *dst)
{
    ...
    if (SCI_rx_packets != 0)
    {
        ...
        do
        {
            ...
            if ((index & 0x0001) == 0)
            {
                *(dst + (index>>1)) =  (*(dst + (index>>1)) & 0x00FF) | (bajt << 8);
            }
            else
            {
                *(dst + (index>>1)) =  (*(dst + (index>>1)) & 0xFF00) | (bajt);
            }
            index = index + 1;
        } while (bajt != 0);
